...
import com.codelabsk.litepipe.browser.YoutubeWebview;
import com.codelabsk.litepipe.downloader.core.DownloadPrefs;
import com.codelabsk.litepipe.downloader.core.DownloadSelectionConfig;
import com.codelabsk.litepipe.downloader.core.DownloadTaskFactory;
...
import com.codelabsk.litepipe.extractor.PlaybackDetails;
...
public final class PlaylistDownloadDialog {
	...
	public void show() {
		...
		if (videoButton != null) {
			videoButton.setOnClickListener(v -> {
				if (isUiLockedForBatch()) return;
				if (primaryMediaMode == DownloadSelectionConfig.PrimaryMediaMode.VIDEO) {
					primaryMediaMode = DownloadSelectionConfig.PrimaryMediaMode.NONE;
				} else {
		...
		if (audioButton != null) {
			audioButton.setOnClickListener(v -> {
				if (isUiLockedForBatch()) return;
				if (primaryMediaMode == DownloadSelectionConfig.PrimaryMediaMode.AUDIO) {
					primaryMediaMode = DownloadSelectionConfig.PrimaryMediaMode.NONE;
				} else {
		...
		if (activeDialog != null) {
			...
			dialogView.post(() -> {
				...
				dialogView.post(() -> {
					if (dialogView.getHeight() > maxHeight) {
	...
	private void showQualitySelectionDialog(boolean isVideo) {
		...
		AlertDialog d = new MaterialAlertDialogBuilder(context)
						.setTitle(isVideo ? R.string.video_quality : R.string.audio_track)
						.setView(v)
						.create();
		...
		String[] options = isVideo 
						? new String[]{"1080p", "720p", "480p", "360p", "240p", "144p"}
						: new String[]{"160kbps", "128kbps", "64kbps", "48kbps"};
		...
		for (String opt : options) {
			...
			if (opt.equals(preferredQuality)) {
				cb.setChecked(true);
				refs[0] = cb;
			}
		}
		
		v.findViewById(R.id.button_confirm).setOnClickListener(v1 -> d.dismiss());
	...
	private void runBatchPreparation(@NonNull BatchRequest batchRequest) {
		...
		String subFolder = taskFactory.sanitizeFileName(batchRequest.parent.getTitle());
		...
		for (int playlistIndex : batchRequest.selectedIndexes) {
			...
			try {
				...
				failedRows++;
				postRowBatchStatus(playlistIndex, PlaylistDownloadItem.BatchResultStatus.FAILED, cause != null ? cause.getMessage() : e.getMessage());
				processedIndexes.add(playlistIndex);
