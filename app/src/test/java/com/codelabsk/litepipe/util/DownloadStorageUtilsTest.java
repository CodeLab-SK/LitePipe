...
public class DownloadStorageUtilsTest {
	...
	public void getOpenUri_usesFileProviderForExistingLocalFile() throws Exception {
		...
		when(context.getPackageName()).thenReturn("com.codelabsk.litube");

		try (MockedStatic<FileProvider> fileProvider = mockStatic(FileProvider.class)) {
			fileProvider.when(() -> FileProvider.getUriForFile(context, "com.codelabsk.litube.provider", file))
							.thenReturn(expected);
	...
	public void getDownloadsLocationLabel_includesApplicationName() {
		final Context context = mock(Context.class);
		when(context.getString(com.codelabsk.litepipe.R.string.app_name)).thenReturn("LiteTube");
